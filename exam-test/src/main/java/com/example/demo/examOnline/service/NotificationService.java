...
public class NotificationService {
            ...
            for (com.example.demo.examOnline.domain.StudentClass sc : studentClasses) {
                try {
                    ...
                    if (notification != null) {
                        successCount++;
                        log.debug("Successfully created notification {} for student {}", notification.getNotificationId(), studentId);
                    } else {
                        log.warn("Failed to create notification for student {} (createNotification returned null)", studentId);
                    }
                } catch (Exception e) {
    ...
    public void checkAndNotifyAssignmentDeadlines(Integer userId) {
        try {
            ...
            List<com.example.demo.examOnline.domain.StudentClass> studentClasses = 
                    studentClassRepository.findByStudentId(userId);
                ...
                for (Post assignment : assignments) {
                    ...
                    if (assignment.getPostType() == com.example.demo.examOnline.domain.enums.PostType.ASSIGNMENT 
                        ...
                        if (dueDate.isAfter(now) && dueDate.isBefore(oneDayLater) || 
                            dueDate.isEqual(now) || (dueDate.isAfter(now) && dueDate.isBefore(oneDayLater.plusHours(1)))) {
                            ...
                            if (!hasSubmitted) {
                                ...
                                boolean notificationExists = notificationRepository
                                        ...
                                        .anyMatch(n -> n.getNotificationType() == NotificationType.ASSIGNMENT_DEADLINE
                                                ...
                                                && n.getCreatedAt().isAfter(now.minusHours(2))); // Check within last 2 hours
